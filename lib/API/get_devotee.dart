...
class GetDevoteeAPI extends DioFuctionAPI {
  ...
  Future<Map<String, dynamic>?> devoteeListBycreatedById(
      String createdById, int page, int limit) async {
    try {
  ...
  Future<Map<String, dynamic>> advanceSearchDevotee(
    ...
    try {
      ...
      if (status == null) {
        if (isAscending == true) {
          response = await getAPI(
              "devotee/advance-search?$searchBy=$encodedQuery&page=$page&limit=$limit&name=ascending");
        } else if (isAscending == false) {
          response = await getAPI(
              "devotee/advance-search?$searchBy=$encodedQuery&page=$page&limit=$limit&name=descending");
        } else {
          response = await getAPI(
        ...
        if (isAscending == true) {
          response = await getAPI(
              "devotee/advance-search?$searchBy=$encodedQuery&advanceStatus=$status&page=$page&limit=$limit&name=ascending");
        } else if (isAscending == false) {
          response = await getAPI(
              "devotee/advance-search?$searchBy=$encodedQuery&advanceStatus=$status&page=$page&limit=$limit&name=descending");
        } else {
          response = await getAPI(
